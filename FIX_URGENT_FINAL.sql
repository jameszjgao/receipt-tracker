-- ============================================
-- 紧急修复：完全解决所有问题
-- 1. 修复 RPC 函数的列名歧义问题（关键！）
-- 2. 暂时禁用 INSERT 策略（强制使用 RPC 函数）
-- 3. 确保 SELECT 策略正确
-- ============================================

-- 第一步：修复 get_user_household_id 函数（确保不查询 users 表）
...
BEGIN
  ...
  -- 检查用户是否是管理员（只查询 user_households 表）
  SELECT EXISTS (
    SELECT 1 
    FROM user_households 
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE '⚠️  Could not change function owner (this is OK): %', SQLERRM;
  END;
END $$;
...
-- 第三步：暂时禁用 INSERT 策略（强制使用 RPC 函数）
-- 这是为了避免 INSERT 策略查询 users 表的问题
DO $$
DECLARE
    r RECORD;
BEGIN
    ...
    FOR r IN (
        ...
        RAISE NOTICE '✅ 删除了 INSERT 策略: %', r.policyname;
    END LOOP;
    
    RAISE NOTICE '✅ 已禁用直接 INSERT（必须使用 RPC 函数）';
END $$;

-- 第四步：确保 SELECT 策略正确（不查询 public.users 表）
DO $$
DECLARE
    r RECORD;
BEGIN
...
SELECT 
    ...
    CASE 
        WHEN COUNT(*) = 0 THEN '✅ 已禁用（必须使用 RPC 函数）'
        ELSE '❌ 仍有 INSERT 策略（应该禁用）'
    END as status,
...
SELECT 
    ...
    CASE 
        WHEN routine_definition LIKE '%RETURNING%' THEN '✅ 函数存在且有 RETURNING 子句'
        ELSE '❌ 函数不存在或格式错误'
    END as status,
    routine_name as policy_names
