...
-- 注意：household_id 和 current_household_id 可以为 NULL（支持两步注册）
CREATE TABLE users (
  id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  email TEXT NOT NULL,
  name TEXT,
  household_id UUID REFERENCES households(id) ON DELETE SET NULL, -- 可空，向后兼容
  current_household_id UUID REFERENCES households(id) ON DELETE SET NULL, -- 当前活动的家庭
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
BEGIN
  ...
  -- 创建用户记录（如果不存在）
  INSERT INTO users (id, email, name, household_id, current_household_id)
  VALUES (p_user_id, p_email, p_user_name, v_household_id, v_household_id)
  ON CONFLICT (id) 
  DO UPDATE SET 
    email = p_email,
...
-- 允许 household_id 和 current_household_id 为 NULL（两步注册）
