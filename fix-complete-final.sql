...
-- 完整修复方案：解决所有问题
-- 1. 修复 RPC 函数的列名歧义
-- 2. 确保 get_user_household_id 函数不查询 users 表
-- 3. 确保 INSERT 策略不查询 users 表
-- ============================================
...
-- 验证函数已修复
SELECT 
    '✅ get_user_household_id 函数' as status,
    CASE 
...
-- 第三步：确保 INSERT 策略正确（不查询 users 表）
DO $$
DECLARE
    r RECORD;
BEGIN
    ...
    -- 创建新的 INSERT 策略（完全不查询 users 表，也不使用可能查询 users 的函数）
...
SELECT 
    ...
    CASE 
        ...
        WHEN with_check LIKE '%get_user_household_id%' THEN '⚠️  使用可能查询 users 的函数'
        ELSE '✅ 不查询 users 表'
    END as status
FROM pg_policies
WHERE schemaname = 'public' 
