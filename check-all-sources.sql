...
-- ============================================
-- 检查 5: 所有可能被 RLS 调用的函数
-- ============================================
SELECT 
    '=== 检查 5: 所有可能查询 users 的函数 ===' as section,
    routine_name,
    security_type,
    routine_definition,
    CASE 
...
WHERE routine_schema = 'public'
  AND (routine_definition LIKE '%household_invitations%'
    OR routine_definition LIKE '%inviter_id%')
  AND (routine_definition LIKE '%users%' OR routine_definition LIKE '%FROM%' OR routine_definition LIKE '%JOIN%');
...
BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        ...
        RAISE NOTICE '错误提示: %', error_hint;
        RAISE NOTICE '错误上下文: %', error_context;
...
-- ============================================
-- 检查 7: 检查所有表约束（包括可能遗漏的）
-- ============================================
SELECT 
    '=== 检查 7: 所有表约束 ===' as section,
    ...
    CASE 
        WHEN pg_get_constraintdef(oid) LIKE '%users%' THEN '❌ 涉及 users 表'
        ELSE '✅ 不涉及 users 表'
    END as status
FROM pg_constraint
